...
export default function CreateRoomSimple({ me, onBack, onCreated }) {
  ...
  function onCreate() {
    const room = createRoom({ name: name || `روم ${Date.now()}`, password: password || null, maxPlayers, botsCount, difficulty, host: me });
    // join as host
  ...
  return (
    <div className="card">
      ...
      <div style={{display:'grid',gap:8,marginTop:12}}>
        ...
        <div style={{display:'flex',gap:8}}>
          <button className="btn-primary btn" onClick={onCreate}>ساخت و ورود</button>
          <button className="btn" onClick={onBack}>بازگشت</button>
        </div>
      </div>
    </div>
  );
